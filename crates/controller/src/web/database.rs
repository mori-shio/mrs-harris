...
async fn database_page(
    ...
    for name in tables_names {
        let is_active = selected_table
            .as_ref()
            .map(|sel| sel == &name)
            .unwrap_or(false);
        tables.push(TableRenderItem { name, is_active });
    }
    ...
    if let Some(ref table_name) = selected_table {
        ...
        let columns_rows = sqlx::query(&col_query).fetch_all(&state.db).await;
