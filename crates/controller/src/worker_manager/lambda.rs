...
pub async fn launch(state: &AppState, run: &JobRun) -> anyhow::Result<String> {
    ...
    let mut is_local = state.config.lambda.function_name == "local" || state.config.lambda.function_name.is_empty();
    ...
    if !is_local {
        match launch_aws_lambda(state, run).await {
            ...
            Err(e) => {
                tracing::warn!("Failed to launch Lambda on AWS: {}. Falling back to local process spawning.", e);
            }
        }
    }
...
async fn launch_aws_lambda(state: &AppState, run: &JobRun) -> anyhow::Result<String> {
    ...
    tracing::info!("Lambda function invoked successfully on AWS. Request ID: {}", request_id);
    Ok(request_id)
}
...
async fn launch_local_process(state: &AppState, run: &JobRun) -> anyhow::Result<String> {
    ...
    tracing::info!("Launching local worker process as Lambda fallback: {:?} worker --task-id {} --callback-url {}", current_exe, task_id, callback_url);
