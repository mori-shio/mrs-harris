...
use mrs_harris_common::models::notification::{ChannelType, EmailConfig, SlackConfig};
...
pub async fn trigger_notifications(
    ...
    let job = match crate::db::jobs::get_job(&state.db, &run.job_id).await? {
        ...
        None => {
            tracing::warn!(
                "Notification triggered for run {} with non-existing job: {}",
                run_id,
                run.job_id
            );
            return Ok(());
    ...
    let channels =
        crate::db::notifications::get_notifications_for_job(&state.db, &job.id, event).await?;
    if channels.is_empty() {
        return Ok(());
    }

    tracing::info!(
        "Sending {} notification(s) for job {} ({}), event: {}",
        channels.len(),
        job.name,
        run_id,
        event
        ...
        match channel.channel_type {
            ChannelType::Slack => {
                ...
                if let Some(ref path) = slack_config.ssm_parameter_path
                    && !path.trim().is_empty()
                {
                    let region = slack_config
                        .ssm_region
                        .as_deref()
                        .unwrap_or("ap-northeast-1");
                    tracing::info!(
                        "SSM パラメータストアから Slack Webhook URL を取得します: {} (リージョン: {})",
                        path,
                        region
                    );
                    match fetch_ssm_parameter(region, path).await {
                        ...
                        Err(e) => {
                            tracing::error!(
                                "SSM パラメータストアからの Slack Webhook URL 取得に失敗しました: {}. 設定済みの固定 URL でフォールバックします。",
                                e
                            );
                        }
                    }
                ...
                if slack_config.webhook_url.is_empty()
                    && let Some(ref global_slack) = state.config.notification.slack
                    && let Some(ref url) = global_slack.default_webhook_url
                {
                ...
                if slack_config.webhook_url.is_empty() {
                    tracing::error!(
                        "Slack notification failed: Webhook URL is empty for channel: {}",
                        channel.name
                    );
                    continue;
                }
                ...
                if let Err(e) = slack::send_slack_notification(&slack_config, &body).await {
                    tracing::error!(
                        "Failed to send Slack notification for channel '{}': {}",
                        channel.name,
                        e
                    );
                }
            ...
            ChannelType::Email => {
                ...
                if let Some(ref global_email) = state.config.notification.email {
                    if let Err(e) =
                        email::send_email_notification(global_email, &email_config, &subject, &body)
                            .await
                    {
                        tracing::error!(
                            "Failed to send Email notification for channel '{}': {}",
                            channel.name,
                            e
                        );
                    }
                    ...
                    tracing::error!(
                        "Email notification failed: Global SMTP configuration is not available"
                    );
                }
            }
        }
