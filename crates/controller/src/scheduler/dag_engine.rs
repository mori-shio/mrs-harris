...
pub fn resolve_and_dispatch(state: AppState, run_id: i64) -> Pin<Box<dyn Future<Output = anyhow::Result<()>> + Send>> {
    ...
    let (parents, children) = {
        ...
        for row in &edges_rows {
            ...
            parents.entry(to_task.clone()).or_default().push(from_task.clone());
        ...
        if status == "unexecuted" || status == "queued" {
            ...
            if let Some(parent_list) = parents.get(task_name) {
                for parent in parent_list {
                    let parent_status = task_runs_map.get(parent).map(|(_, s)| s.as_str()).unwrap_or("unexecuted");
    ...
    for (task_name, tr_id, worker_type) in tasks_to_launch {
        ...
        tokio::spawn(async move {
            if let Err(e) = launch_task_worker(state_clone.clone(), run_id_clone, tr_id, &task_name_clone, worker_type).await {
                tracing::error!("Failed to launch worker for task {} in DAG run {}: {}", task_name_clone, run_id_clone, e);
...
fn mark_downstream_skipped(
    ...
    while let Some(current) = queue.pop_front() {
        let current_status = task_statuses.get(&current).map(|s| s.as_str()).unwrap_or("unexecuted");
...
async fn complete_dag_run(
    ...
    if let Some(latest) = latest_run_opt {
        ...
        let duration_ms = latest.started_at.map(|s| {
