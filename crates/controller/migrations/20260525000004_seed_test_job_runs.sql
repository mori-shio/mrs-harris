...
(
    '101',
    1,
    '{"タグ": ["test", "local"], "説明": "設定バージョン履歴 (v1/v2) の比較検証用OneShotジョブ", "ジョブ名": "test-job", "ジョブタイプ": "OneShot (単発/手動実行)", "タイムアウト": "3600 秒", "有効化状態": "有効", "初期遅延": "10 秒", "リトライ上限": "3", "バックオフ戦略": "指数", "ワーカー定義": "default-fargate", "スクリプト": "echo hello world", "Slack通知": {"成功時": "無効", "失敗時": "無効", "ジョブ起動時": "無効"}}',
    'system-admin',
...
(
    '101',
    2,
    '{"タグ": ["test", "local", "production"], "説明": "設定バージョン履歴 (v1/v2) の比較検証用OneShotジョブ (v2拡張)", "ジョブ名": "test-job", "ジョブタイプ": "OneShot (単発/手動実行)", "タイムアウト": "3600 秒", "有効化状態": "有効", "初期遅延": "10 秒", "リトライ上限": "3", "バックオフ戦略": "指数", "ワーカー定義": "default-fargate", "スクリプト": "echo hello world --verbose", "Slack通知": {"成功時": "無効", "失敗時": "有効", "ジョブ起動時": "無効"}}',
    'user-shion',
...
(
    '102',
    1,
    '{"タグ": ["backup", "cron"], "説明": "5分毎に起動して定期バックアップを行う自動化ジョブ", "ジョブ名": "cron-fargate-job", "ジョブタイプ": "Cron (定期実行)", "スケジュール (Cron)": "0 */5 * * * *", "タイムアウト": "1800 秒", "有効化状態": "有効", "初期遅延": "30 秒", "リトライ上限": "2", "バックオフ戦略": "固定", "ワーカー定義": "default-fargate", "スクリプト": "echo \\"Running scheduled DB backup...\\"", "Slack通知": {"成功時": "有効", "失敗時": "有効", "ジョブ起動時": "無効"}}',
    'system-admin',
...
(
    '103',
    1,
    '{"タグ": ["pipeline", "lambda"], "説明": "Lambda上で3つの依存タスクを直列実行するデータ収集DAG", "ジョブ名": "dag-lambda-job", "ジョブタイプ": "DAG (タスクパイプライン)", "タイムアウト": "600 秒", "有効化状態": "有効", "初期遅延": "5 秒", "リトライ上限": "1", "バックオフ戦略": "線形", "ワーカー定義": "default-lambda", "スクリプト": [{"name": "task-A", "payload": {"command": "echo A"}, "worker_type": "lambda"}, {"name": "task-B", "payload": {"command": "echo B"}, "worker_type": "lambda"}, {"name": "task-C", "payload": {"command": "echo C"}, "worker_type": "lambda"}], "Slack通知": {"成功時": "無効", "失敗時": "有効", "ジョブ起動時": "無効"}}',
    'system-admin',
...
(
    '104',
    1,
    '{"タグ": ["ml", "heavy"], "説明": "大容量大プロセッサカスタム定義(custom-fargate-large)で実行するタスク", "ジョブ名": "custom-node-job", "ジョブタイプ": "OneShot (単発/手動実行)", "タイムアウト": "7200 秒", "有効化状態": "有効", "初期遅延": "10 秒", "リトライ上限": "3", "バックオフ戦略": "指数", "ワーカー定義": "custom-fargate-large", "スクリプト": "python train.py --epochs 10", "Slack通知": {"成功時": "無効", "失敗時": "無効", "ジョブ起動時": "無効"}}',
    'user-shion',
